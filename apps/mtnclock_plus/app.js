var data = require("Storage").readJSON("mtnclock_plus.json", 1) || {};
let config = require("Storage").readJSON("mtnclock_plus.settings.json") || {};
...
g.clear();
	...
	//mountains
	drawMtn({mtn1:color.mtn2, mtn2:color.mtn1}, {x:px(28), y:py(25)}, {w:px(38), h:py(17)});
	drawMtn({mtn1:color.mtn2, mtn2:color.mtn1}, {x:px(45), y:py(28)}, {w:px(38), h:py(17)});
		...
		if (config.rows) {
			config.rows.forEach((row) => {
				...
				case "clock":
					...
					text = row.name + " | " + require("locale").time(dx, 1).replace(" ", "") + " " + require('locale').dow(dx, 1);
					break;
				case "iqair":
					var aqi = data.aqi.find((e) => e.url == row.url);
					text = row.name + " | " + ((aqi && aqi.aqius) || "...");
					break;
				case "iqairt":
					var aqi = data.aqi.find((e) => e.url == row.url);
					text = row.name + " | " + ((aqi && aqi.aqius) || "...") + " | " + ((aqi && require('locale').temp(aqi.temp).replace(".0", "")) || "...");
					break;
				default:
					return;
				}
				g.setFont("Vector", py(10)).drawString(text, px(2), py(start - rowCount * 10));
				rowCount++;
			});
		}

		// Local Date and Weather
		g.setFont("Vector", py(10)).drawString(require('locale').dow(d, 1)+" "+d.getDate()+" "+require('locale').month(d, 1)+((data.temp == undefined) ? "" : " | "+require('locale').temp(Math.round(data.temp-273.15)).replace(".0", "")), px(2), py(start - rowCount * 10));
		// Time
		g.setFont("Vector", py(20)).setFontAlign(-1, -1).drawString((require("locale").time(d, 1).replace(" ", "")), px(2), py(start - ((rowCount + 2)*10)));
...
function checkAQI() {
	...
	if (config.apiKey && config.rows && (data.aqiTime === undefined || data.aqiTime + 3600000 < d.getTime())) {
		data.aqi = [];
		config.rows.forEach((row) => {
			if ((row.mode == "iqair" || row.mode == "iqairt") && row.url) {
				var el = row.url.replace(/\/$/, "").split("/").reverse();
				var url = `https://api.airvisual.com/v2/city?city=${el[0]}&state=${el[1]}&country=${el[2]}&key=${config.apiKey}`
				Bangle.http(url).then(r => {
					var resp = JSON.parse(r.resp);
					...
					require("Storage").writeJSON('mtnclock_plus.json', data);
				});
			}
		});
	}
